...
These methods cover *reading* strings. On the write side there are two facilities: `DatasetBuilder::with_vlen_strings` writes a contiguous variable-length UTF-8 string dataset, and the attribute variant `AttrValue::VarLenAsciiArray` writes a variable-length ASCII string array as an attribute (see [Groups & attributes](groups-attributes.md)). Both store their payloads in one global heap collection, which caps either at 65,535 elements, and a chunked, filtered, or resizable variable-length-string dataset is refused; see [Limitations](../reference/limitations.md#writing). Reading has neither restriction. For how string datatypes map between HDF5 and this crate, see [Data types](../reference/data-types.md).
